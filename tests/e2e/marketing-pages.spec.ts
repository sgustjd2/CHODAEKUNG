 ...
 * runtime errors, key links, and no *critical* WCAG A/AA violations (serious contrast on the brand
 * palette is deferred/logged, consistent with a11y.spec).
 */
...
async function expectNoCriticalA11y(page: Page, url: string) {
  await page.waitForTimeout(300);
  const { violations } = await new AxeBuilder({ page }).withTags(["wcag2a", "wcag2aa", "wcag21a", "wcag21aa"]).analyze();
  const critical = violations.filter((v) => v.impact === "critical");
  const serious = violations.filter((v) => v.impact === "serious");
  if (serious.length) console.log(`[a11y] ${url}: ${serious.reduce((n, v) => n + v.nodes.length, 0)} serious node(s) (deferred)`);
  expect(critical.map((v) => v.id), `critical a11y violations on ${url}`).toEqual([]);
}

test("/guide renders with a create CTA and no critical a11y", async ({ page }) => {
  ...
  await expectNoCriticalA11y(page, "/guide");
});

test("/privacy renders the policy and no critical a11y", async ({ page }) => {
  const errors = trackJsErrors(page);
  ...
  expect(errors, `runtime errors:\n${errors.join("\n")}`).toEqual([]);
  await expectNoCriticalA11y(page, "/privacy");
});
