...
## 2026-09-23 (27) — 공개 페이지 JS 33% 감량 (인증 클라이언트·QR 지연 로드)

브라우저가 실제로 받는 JS를 측정해 보니 **모든 공개 페이지가 Supabase 인증 클라이언트(약 67KB gzip, 전체 JS의 약 30%)를 처음부터 로드** — 랜딩·템플릿·초대장 뷰어까지. `supabase-browser.ts`가 `@supabase/ssr`을 정적 import해서, 로그아웃 버튼(랜딩 헤더)이나 뷰어의 RSVP 이름 자동 채우기처럼 "상호작용 후에만" 쓰는 곳 때문에 전 페이지가 비용을 냄. 뷰어는 개발자 테마 전용 QR 라이브러리(7KB)도 항상 받음.

- `createBrowserSupabase()`를 async로 — 라이브러리를 호출 시점에 동적 import(어떤 파일도 다시 즉시 로드 못 하게 구조적으로). 호출부 11곳 await.
- 뷰어 RSVP 이름 자동 채우기: 마운트 시 → **RSVP 창을 처음 열 때**(이름 칸이 보이는 유일한 곳). 읽기만 하는 하객은 인증 클라이언트를 안 받음.
- QR 저장: 탭할 때 동적 import.

결과(gzip, 초기 로드): 뷰어 224.6→150.4KB(−33%), 랜딩·템플릿 216.5→149.8KB(−31%). 신규 `bundle-budget.spec`: 공개 페이지 초기 JS에 인증/QR 라이브러리가 없고 180KB 미만인지 + QR 저장이 여전히 PNG를 내려받으며 라이브러리를 탭 시에만 가져오는지 검증. 전체 238 passed.

## 2026-09-23 (28) — 업로드 사진 자동 축소 (갤러리 원본 그대로 올라가던 문제)

커버는 크롭기가 1600px JPEG로 다시 인코딩하지만, **갤러리 사진은 원본 그대로** 업로드되어 휴대폰 사진(약 4000px, 3~5MB)을 모든 하객이 원본 크기로 받았고, 5MB를 넘는 사진은 업로드 자체가 거절됐음.
...
## 2026-09-23 (30) — 에디터(작성 화면) 접근성: 입력칸 이름·숨김 시트 포커스·대비

그동안 a11y 게이트는 하객용 공개 페이지만 봤음. 에디터를 처음 스캔하니:
- **critical `label` (~52개):** 인스펙터의 모든 입력칸에 프로그램적 이름이 없음 — `Field`가 보이는 캡션을 입력칸과 연결하지 않아 스크린리더엔 "편집 텍스트"만 반복. 에디터를 사실상 쓸 수 없는 수준.
- **serious `aria-hidden-focus`:** 닫힌 바텀시트·발행 오버레이가 aria-hidden인데 안의 컨트롤에 Tab으로 들어감.
- **serious 대비:** 에디터 크롬 전반(muted·--wax-deep 문구, 세이지 "SAVED" 2.57), 발행 다이얼로그 포함.

수정: `Field` 래퍼를 실제 `<label>`로(캡션=접근성 이름, 캡션 탭 시 포커스), 개별 입력 4곳(행사 일시·정원 ×데스크톱/모바일)과 제목 입력에 aria-label, 숨김 시트·오버레이에 `inert`(포커스·보조기술 모두에서 제외), 에디터·발행 다이얼로그 규칙 23곳에 공개 페이지와 같은 대비 처방(muted→--ink-3, --wax-deep 문구→--wax-onpage, 세이지 상태 문구→#3F5A3B, 선택 태그 채움→--wax-onpage).

게이트: 신규 `a11y-editor.spec` — 모바일 에디터+바텀시트 4개, 데스크톱 인스펙터 탭 4개, 발행 다이얼로그 (에디터 안 미리보기는 뷰어 게이트가 담당하므로 제외). 열리는 애니메이션 중에 스캔하면 반투명 글자로 가짜 대비 위반이 잡혀서, 유한 애니메이션이 끝날 때까지 기다린 뒤 스캔(sleep 아님). 3회 반복 병렬 9/9, 전체 248 passed.

## 2026-09-23 (31) — 로그인 뒤 앱 화면(대시보드·응답함·관리자·설정·미디어) 대비

공개 페이지·에디터와 달리 로그인 뒤 화면은 a11y 게이트 밖이었음. 로그인 없이 렌더되는 `/rsvp` 빈 상태를 스캔하니 serious 대비 위반(안내 문구 muted 3.41). 같은 CSS를 쓰는 나머지 화면을 규칙 단위로 전수 점검 → 기준 미달 문구 ~50곳.

수정(공개 페이지와 같은 처방): muted→`--ink-3`, `--wax-deep` 문구→`--wax-onpage`, 아바타 채움→`--wax-onpage`. 같은 색조의 옅은 배경 위 배지 글자는 `--wax-onpage`로도 ~4.3이라 토큰 3개 추가: `--wax-on-tint` #A8474A(코랄 배지), `--sage-ink` #3F5A3B(세이지 배지·"참석" 수치), `--lilac-ink` #5B4A78(라일락/"미정" 배지·수치). 응답함 통계 큰 숫자(참석/미정)는 44px여도 2.6:1이라 같은 잉크로. 대시보드 탭 개수(`.c`) 반투명은 활성 탭(어두운 배경)에서만. 도넛 차트·범례 색 칩은 그래픽(숫자는 텍스트로 병기)이라 그대로.

한계: `/rsvp` 빈 상태 외에는 로그인이 필요해 axe로 실측 못 함(계정 생성 불가) → 토큰 쌍을 unit `contrast.spec`에 고정(실제 배지 rgba를 흰 카드에 합성해 계산), 어두운 통계 카드(.stat.featured/.stat.dark)는 더 높은 명시도로 기존 색을 유지함을 규칙으로 확인. `/rsvp`(빈 상태)를 `a11y.spec` 페이지 게이트에 추가. 전체 256 passed.
