...
| Content editors (shared) | `src/components/editor/content-editors.tsx` | client; `<ContentEditors draft patch>` — **one** set of per-section editors used by BOTH desktop inspector and mobile sheet (no duplication/drift). Covers every section type except matchInfo/champions. Emits `.insp-*` (styled under `.editor-page` for both). | done |
