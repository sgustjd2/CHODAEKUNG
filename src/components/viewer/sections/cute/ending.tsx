...
export function CuteEnding({ content }: { content: EndingContent }) {
  return (
    <div className="c-ending">
      ...
      {content.signature && (
        <div className="say">
          <Editable path="signature">{content.signature}</Editable> <Icon name="ic-heart-fill" className="c-heart" width={20} height={20} />
        </div>
      )}
      {content.names && <div className="from"><Editable path="names">{content.names}</Editable></div>}
