...
function ContentPanel({ api }: { api: EditorApi }) {
  ...
  return (
    <>
      ...
      <ContentEditors draft={api.draft} patch={api.patch} selectedId={api.selectedId} />
      ...
      <div className="insp-group">
        ...
        <div className="insp-field">
          ...
          <input
            ...
            onChange={(e) => { const n = parseInt(e.target.value, 10); api.setDraft((d) => ({ ...d, capacity: Number.isFinite(n) && n > 0 ? n : undefined })); }}
          />
          <div style={{ fontSize: 11, color: "var(--fg-3, #8a8a99)", marginTop: 6, lineHeight: 1.5 }}>
      ...
      <div className="m-group">
        ...
        <div className="m-radios">
          {([["좁게", -0.02], ["기본", 0], ["넓게", 0.04]] as const).map(([label, n]) => (
            <button key={label} type="button" className={`m-radio${letterSpacing === n ? " active" : ""}`} onClick={() => setLetterSpacing(n)}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div className="m-group">
        ...
        <div className="m-radios">
          {([["좁게", 0.92], ["기본", 1], ["넓게", 1.12]] as const).map(([label, n]) => (
            <button key={label} type="button" className={`m-radio${lineHeight === n ? " active" : ""}`} onClick={() => setLineHeight(n)}>
              {label}
            </button>
          ))}
        </div>
